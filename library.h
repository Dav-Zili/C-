...
/*
int scambia_numeri(int  & a ,int & b)
{
	int c = a;
	 a =  b;
	 b = c;
...
void ordinamento_ingenuo(int v[] , int n )
{
	for(int i = 0; i < n-1 ; i++)
	{
		for(int j=i+1 ; j < n ; j++)
		{
			if(v[i]>v[j])
			{
				scambia_numeri(v[i] , v[j] );

			}
		}
	}	
}
int togli_elemento(int v[] , int n , int x)
{
	int elementi_tolti = 0;
	for(int i = 0 ; i < n ; i++)
	{
		if(v[i] == x)
		{
...
void stampa_vettore_float(float v[] , int n)
{
	for (int i = 0 ; i!= n ; i++)
	{
		cout << "Elemento " << i + 1  << " = " << v[i] << endl ;
...
void riempi_vettore_float(float v[] , int n , int x , int y)
{
	for(int i = 0 ; i < n ; i++)
	{
		v[i] = random_float(x,y);
	}
}
...
void stampasece(int v[] , int n )
{	
	for (int i = 0 ; i <= n  ; i++ )
	{	
		cout << i << " " << "--> "  ; 
		if (ricerca_binaria(v , n , i)) 
		{
			cout << "Presente" << endl ;
		}
		else 
...
void dimmisece(int v[] , int n , int x)
{
	if(ricerca_binaria( v , n, x))
	{
		cout << "Il numero " << x << " c'e' " << endl;
	}
	else
	{
		cout << "Non c'e'" << endl ; 
...
void ricerca_normale(int v[] , int n , int x)
{
	for(int i = 0; i < n ; i++)
	{
		if(v[i]==x)
